...
*/
export function filterRoles(roles: Array<number>) {
  return (req: Request, res: Response, next: Function) => {
    if (req.session == null) req.session = {};
    let roleId = req.session.jwt.roleId;
    if (roleId == null) return Controller.unauthorized(res);
    if (roles.indexOf(roleId) < 0) return Controller.unauthorized(res);
    next();
  };
}
