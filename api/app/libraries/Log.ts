...
// Setting up logger
const logDir = path.join(process.env.HOME, ".back-logs");
if (!fs.existsSync(logDir)) {
  fs.mkdirSync(logDir);
}

SegfaultHandler.registerHandler(path.join(logDir, "back-segfault.log"));
...
log.add(winston.transports.File, {
  name: "error-file",
  level: "error",
  filename: path.join(logDir, "back-error.log"),
  json: true
});
...
log.add(winston.transports.File, {
  name: "warn-file",
  level: "warn",
  filename: path.join(logDir, "back-warn.log"),
  json: true
});
...
log.add(winston.transports.File, {
  name: "debug-file",
  level: "debug",
  filename: path.join(logDir, "back-debug.log"),
  json: true
});
...
requestLog.add(winston.transports.File, {
  name: "request-file",
  level: "info",
  filename: path.join(logDir, "back-requests.log"),
  json: true
});
